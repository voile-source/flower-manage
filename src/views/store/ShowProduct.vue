      ...
      <div class="fl-table" v-if="tableData.length != 0">
        <el-table :data="tableData" style="width: 100%" :stripe="true">
          ...
          <el-table-column label="商品名称" align="center">
            <template slot-scope="props">
              <span class="fl-article-desc">{{ props.row.name }}</span>
            </template>
          </el-table-column>
          <el-table-column label="商品图片" align="center">
            <template slot-scope="props">
              <img style="width:140px;height:85px;" :src="props.row.cover">
            </template>
          </el-table-column>
          <el-table-column label="商品价格" align="center">
            <template slot-scope="props">
              <span class="fl-article-desc">{{ props.row.price }}</span>
            </template>
          </el-table-column>
          <el-table-column label="商品库存" align="center">
            <template slot-scope="props">
              <span class="fl-article-desc">{{ props.row.stock }}</span>
            </template>
          </el-table-column>
          <el-table-column label="操作" align="center">
            <template slot-scope="scope">
              ...
              <el-popver
                ...
                title="是否通过审核？"
                placement="top"
                @confirm="handelConfirm(scope.$index, scope.row)"
              >
                <el-button
                  style="margin-left: 1rem"
                  size="mini"
    ...
    <el-dialog title="查看文章" :visible.sync="show_dialog" width="150rem">
      <el-form label-position="left" inline class="demo-table-expand">
        ...
        <el-form-item label="文章内容">
          <div
            style="box-sizing: border-box; padding: 0 10rem 0 0; font-size: 1.6rem"
            v-html="showArticle.html"
          ></div>
        </el-form-item>
      </el-form>
    </el-dialog>
    ...
    init() {
      this.$apiFun.getProductList({
        ...
        .then((res) => {
          ...
          this.tableData = res.data.exchangeList;
